...
# 常见占位符建议选项（快速填充）
_SUGGEST_OPTIONS: Dict[str, List[str]] = {
    "主体": ["人物", "风景", "静物", "建筑", "动物"],
    "风格": ["写实", "赛博朋克", "日系", "极简", "复古", "插画", "摄影"],
    "光照": ["自然光", "逆光", "侧光", "柔光", "霓虹光", "黄金时段"],
    "构图": ["三分法", "居中", "对称", "低机位", "高机位", "特写", "广角"],
    "画幅": ["16:9", "4:3", "1:1", "3:2", "9:16"],
    "材质": ["金属", "玻璃", "木质", "皮革", "大理石", "织物"],
    "色调": ["冷色调", "暖色调", "高对比", "柔和", "单色", "霓虹"],
    "tags": ["use selected tags"],
}
...
class ExpandPanel(ctk.CTkToplevel):
    def __init__(self, parent, initial_text: str,
        ...
        # 占位符变量映射
        self.placeholder_vars: Dict[str, tk.Variable] = {}
        self.placeholder_container: Optional[ctk.CTkFrame] = None
    ...
    def _build_ui(self, initial_text: str):
        ...
        # 变量占位符区域
        ph_section = ctk.CTkFrame(main)
        ph_section.pack(fill="x", padx=6, pady=(0, 8))
        top_bar = ctk.CTkFrame(ph_section)
        top_bar.pack(fill="x")
        ctk.CTkLabel(top_bar, text="变量占位符：在预设中用 {变量名} 标记可变部分，下面可快速填写").pack(side="left", padx=6, pady=(6, 0))
        if self.get_selected_tags_cb:
            ctk.CTkButton(top_bar, text="用标签填充常用项", width=140, command=self._fill_from_tags).pack(side="right", padx=6, pady=(6, 0))
        self.placeholder_container = ctk.CTkFrame(ph_section)
        self.placeholder_container.pack(fill="x", padx=6, pady=6)
        self._rebuild_placeholder_inputs()
    ...
    def _rebuild_placeholder_inputs(self):
        # 清空容器
        if self.placeholder_container is None:
            return
        # 使用标准API清空子控件，避免访问私有属性导致异常
        ...
        tpl = self.txt_preset.get("1.0", "end").strip()
        placeholders = self._extract_placeholders(tpl)
        if not placeholders:
            ctk.CTkLabel(self.placeholder_container, text="未检测到占位符，可直接生成。").pack(anchor="w")
            return

        grid = ctk.CTkFrame(self.placeholder_container)
        grid.pack(fill="x")
        for i, name in enumerate(placeholders):
            ctk.CTkLabel(grid, text=name).grid(row=i, column=0, padx=6, pady=4, sticky="w")
            values = _SUGGEST_OPTIONS.get(name)
            if values:
                var = tk.StringVar()
                cb = ctk.CTkComboBox(grid, values=values, variable=var)
                cb.grid(row=i, column=1, padx=6, pady=4, sticky="we")
                if values:
                    cb.set(values[0])
                self.placeholder_vars[name] = var
            else:
                var = tk.StringVar()
                entry = ctk.CTkEntry(grid, textvariable=var)
                entry.grid(row=i, column=1, padx=6, pady=4, sticky="we")
                self.placeholder_vars[name] = var
            grid.grid_columnconfigure(1, weight=1)

    def _fill_from_tags(self):
        if not self.get_selected_tags_cb:
            return
        try:
        ...
        head_join = ", ".join(head)
        tail_join = ", ".join(tail)
        both_join = ", ".join([*head, *tail])
        # 常见映射
        mapping = {
            "风格": head_join or both_join,
            "元素": tail_join or both_join,
            "tags": both_join,
        }
        for k, v in mapping.items():
            var = self.placeholder_vars.get(k)
            if var:
                var.set(v)
    ...
    def _on_generate(self):
        ...
        # 根据占位符输入进行替换
        resolved_preset = base_preset
        try:
            for name, var in self.placeholder_vars.items():
                val = (var.get() or "").strip()
                pattern = r"\{" + re.escape(name) + r"\}"
                resolved_preset = re.sub(pattern, val, resolved_preset)
            # 清除未填充的占位符
            resolved_preset = re.sub(r"\{[^{}]+\}", "", resolved_preset)
        except Exception:
            resolved_preset = base_preset
            ...
            try:
                ...
                for i in range(n):
                    sys_preset = _build_system_preset(
                        ...
                        use_tags=True, tags=tags, use_negative=use_negative,
...
class PresetEditDialog(ctk.CTkToplevel):
    ...
    def _build_ui(self):
        ...
        # 提示信息
        tip = ctk.CTkLabel(main, text="提示：可使用 {变量名} 作为占位符，如 {主体}、{风格} 等", 
                          text_color="gray")
